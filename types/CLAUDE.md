...
| `cards.ts` | `CardRarity`, `CardEffect` (discriminated union: `CardStatFlatEffect`, `CardStatPercentEffect`, `CardTriggerEffect`, `CardStatusResistEffect`), `TIER_TO_RARITY`, `BestiaryUnlockTier`, `BESTIARY_THRESHOLDS`, `BestiaryEntry`, `BestiaryCardInfo` (com `userCardPurity` e `cardArtUrlSpectral`), `BestiaryResponse`, `UserCardSummary` (com `purity`, `spectralSkillId?` e `card.cardArtUrl?` / `card.cardArtUrlSpectral?`), `AbsorbResponse` (resposta do POST `/api/cards/[id]/absorb`), `SpectralDropBroadcast` (payload do evento `global:spectral-drop`), `ShowcaseResponse` ({ userCardIds, cards }) |
