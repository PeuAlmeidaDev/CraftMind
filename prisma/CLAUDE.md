...
| `UserCard` | Copia de cristal pertencente a um usuario: userId, cardId, equipped, slotIndex (0/1/2 quando equipped), xp (Int default 0), level (Int default 1, range 1-5), **purity** (Int default 50, range 0-100 — IV do cristal; baseline 50 = 1.0x; 100 = Espectral), **spectralSkillId** (String?, FK para Skill, onDelete SetNull — skill espectral escolhida pelo dono; so aplica quando purity === 100, habilita o 5o slot em batalha; valida que pertence aos 4 mob skills do mob de origem na rota PUT). Constraint unique em `[userId, slotIndex]` (dois cristais nao ocupam o mesmo slot). Indice composto `[userId, cardId]` acelera "minhas copias deste cardId" (multiplas copias permitidas). Cada drop SEMPRE cria UserCard novo — duplicatas viram XP apenas via sacrificio manual (`POST /api/cards/[id]/absorb`): cada source contribui `XP_PER_DUPLICATE_BY_RARITY[rarity]` (50/100/200/400/800 por raridade) ao alvo (mesma curva da Fase 1; thresholds 100/250/500/1000; multiplicadores 1.0/1.2/1.4/1.6/1.8). Effects sao escalonados por `purity/50 * levelMultiplier`. Cards 3⭐ tem floor de purity 30 no drop. Indices em userId, `[userId, cardId]` e spectralSkillId |
