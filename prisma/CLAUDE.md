...
| `Skill` | Habilidade desbloqueável: name, description, tier, cooldown, target (String — validado na aplicacao com SkillTarget), damageType, basePower, hits, accuracy (Int, default 100 — % base de acerto, 1-100), effects (JSON — SkillEffect[]), mastery (JSON — SkillMastery). Back-relation `userCardsSpectral UserCard[]` (Cristais Espectrais que escolheram esta skill como 5o slot via `UserCard.spectralSkillId`) |
...
| `Mob` | Inimigo PvE: name (unique), description, tier (1-5), aiProfile (AGGRESSIVE/DEFENSIVE/TACTICAL/BALANCED), imageUrl (String?, URL Cloudinary da imagem do mob), maxStars (Int, default 1 — 1 = mob comum sem variacao; 2 ou 3 = pode aparecer em versao Heroi/Lendaria com stats escalados), stats (physicalAtk, physicalDef, magicAtk, magicDef, hp, speed). Relacao N-1 com `Card` (varias Cards por Mob, uma por requiredStars) |
| `Card` | Cristal de Memoria (drop pos-batalha): mobId (relacao N-1 com Mob), name (unique), flavorText, rarity (CardRarity enum), cardArtUrl (String?, URL Cloudinary), **cardArtUrlSpectral** (String?, URL Cloudinary da arte alternativa exclusiva da versao Espectral; quando null, frontend faz fallback CSS holografico sobre cardArtUrl), effects (JSON CardEffect[] em types/cards.ts), dropChance (Float, default 5 — percentual individual de drop quando a variante e elegivel, 0-100), requiredStars (Int, default 1 — estrela minima do encontro para esta variante cair, 1/2/3). Constraint unique em `[mobId, requiredStars]` (uma variante por estrela por mob) |
| `UserCard` | Copia de cristal pertencente a um usuario: userId, cardId, equipped, slotIndex (0/1/2 quando equipped), xp (Int default 0), level (Int default 1, range 1-5), **purity** (Int default 50, range 0-100 — IV do cristal; baseline 50 = 1.0x; 100 = Espectral), **spectralSkillId** (String?, FK para Skill, onDelete SetNull — skill espectral escolhida pelo dono; so aplica quando purity === 100, habilita o 5o slot em batalha; valida que pertence aos 4 mob skills do mob de origem na rota PUT). Constraints unique em `[userId, cardId]` (primeira copia) e `[userId, slotIndex]`. Duplicatas coletadas viram XP/level (50/100/200/400/800 XP por raridade; thresholds 100/250/500/1000; multiplicadores 1.0/1.2/1.4/1.6/1.8). Effects sao escalonados por `purity/50 * levelMultiplier`. Cards 3⭐ tem floor de purity 30 no drop. Indices em userId e spectralSkillId |
| `PendingCardDuplicate` | Drop de duplicata com purity MAIOR aguardando decisao do jogador: id (cuid), userId, userCardId, newPurity (Int 0-100), createdAt. Indices em `[userId, createdAt]` e `[userCardId]`. onDelete Cascade em ambas as relacoes (User e UserCard). Resolucao via `POST /api/cards/pending-duplicates/[id]/resolve`: REPLACE zera xp/level e adota newPurity; CONVERT aplica `applyXpGain` mantendo purity atual. Sem expiracao (acumulam indefinidamente) |
| `SpectralDropLog` | Append-only de cada Cristal Espectral (purity === 100) dropado: id (cuid), userId, userCardId, droppedAt (default now()). Indices em `[droppedAt]` e `[userId]`. onDelete Cascade em ambas as relacoes. Criado dentro da transacao do drop em `lib/cards/drop.ts` quando UserCard NOVO e criado com purity 100. Duplicatas que viram Espectral via REPLACE da pendencia NAO geram log retroativo. Alimenta feed/admin futuros (hall da fama). |
| `UserShowcase` | Vitrine publica de cristais escolhidos pelo dono: id (cuid), userId (unique), userCardIds (String[], max 6 validado em PUT `/api/user/showcase`), createdAt, updatedAt. onDelete Cascade com User. Ordem do array preservada na renderizacao em `/character/[id]/showcase`. Espectrais aparecem com glow dourado animado. |
| `MobKillStat` | Projecao agregada do bestiario: userId, mobId, victories, defeats, damageDealt, firstSeenAt, lastSeenAt. Constraint unique em `[userId, mobId]`. Indices em userId e mobId |
...
| `LoginLog` | Registro append-only de cada login (anti multi-account): userId, visitorId (FingerprintJS, ou "unknown"), ip, userAgent (VarChar 500), loggedInAt. Sem `createdAt`/`updatedAt` por design. Indices em `[userId, loggedInAt]` e `[visitorId, loggedInAt]`. onDelete Cascade com User (apaga junto na exclusao da conta — LGPD). Retencao 90 dias via script `lib/jobs/cleanup-login-log.ts` (executar manualmente: `npx tsx lib/jobs/cleanup-login-log.ts`) |
