...
//
// Politica de drop (multiplas copias permitidas):
// - Toda variante que passa no roll cria UM UserCard NOVO com purity rolada
//   via `rollPurity` (floor de 30 aplicado para variantes 3⭐).
// - Nao ha mais limite de 1 copia por jogador por cardId. Conversao de XP
//   acontece manualmente via sacrificio no inventario (POST /api/cards/[id]/absorb).
// - Sem PendingCardDuplicate.
//
...
// { cardDropped: null, spectralDrop: null }.
...
export type ApplyCardDropAndStatsResult = {
  /** Card dropada (sempre nova UserCard). null caso nenhuma variante tenha
   *  passado no roll de dropChance. */
   ...
   *  transacao (fire-and-forget). null em qualquer outro cenario. */
  spectralDrop: {
    userCardId: string;
    cardName: string;
    mobName: string;
...
/**
 * Atualiza MobKillStat e (se aplicavel) cria UserCard novo em uma transacao
 * Prisma.
 *
 * Regras:
 ...
 * - Para na primeira variante que passa no roll: cria UserCard novo (sem
 *   verificar duplicatas — multiplas copias do mesmo cardId sao permitidas).
 *
...
export async function applyCardDropAndStats(
  ...
  if (!isVictory) {
    return { cardDropped: null, spectralDrop: null };
  }
  ...
  //    passa no roll. Sempre cria UserCard novo — sem checagem de duplicata.
  for (const card of eligibleCards) {
    ...
    // Rola a purity da nova copia.
    const newPurity = rollPurity(rng, card.requiredStars);

    // Cria UserCard novo. Multiplas copias do mesmo cardId sao permitidas.
