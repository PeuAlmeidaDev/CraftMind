...
export default function EditSkillPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/skills/${id}`)
      ...
      .then((json) => {
        ...
        setTier(String(s.tier));
        setCooldown(String(s.cooldown));
        setTarget(s.target);
        setDamageType(s.damageType);
        setBasePower(String(s.basePower));
        setHits(String(s.hits));
        setAccuracy(String(s.accuracy));
        setEffects(JSON.stringify(s.effects, null, 2));
        setMastery(JSON.stringify(s.mastery, null, 2));
      })
      .catch(() => showToast("Erro ao carregar skill", "error"))
  ...
  if (notFound) {
    return <p className="text-gray-500 text-center py-12">Skill nao encontrada</p>;
  }

  return (
    <div className="max-w-3xl">
      <h2 className="text-xl font-bold text-white mb-6">Editar Skill</h2>
