...
| `CardPickerModal.tsx` | Modal para escolher cristal a equipar num slot. Lista candidatos com badges de raridade/tier/`Lv N`/`% PURO`, mini retrato do mob, nome, flavorText, `CardLevelBar` por candidato e bonuses ja escalonados (purity x level). Filtros por raridade (toggle). Focus trap + Escape. Pode listar multiplas copias do mesmo cardId (uma linha por UserCard). |
